...
- Integrated `CA\Log` (`groupesti/laravel-ca-log`) for structured logging across all services and rules.
- Policy evaluation logging via `CaLog::log('policy_evaluation', ...)` in `PolicyEngine` methods (`evaluate`, `validateIssuance`, `validateSubject`, `getEffectivePolicy`) and all built-in rule classes.
- Policy violation logging via `CaLog::log('policy_violation', ...)` whenever a rule or constraint check fails.
- Critical error logging via `CaLog::critical()` with exception details in all service and rule `evaluate` methods; exceptions are re-thrown after logging.
- `NameConstraintValidator::validate()` now logs each individual name constraint violation and emits a summary evaluation log entry.

### Changed

- Added `groupesti/laravel-ca-log: ^0.1` as a required dependency in `composer.json`.
