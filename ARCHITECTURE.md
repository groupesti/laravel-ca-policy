...
- **Structured logging via CA\Log** (2026-03-29): All services and rules log through `CaLog` facade using two operation types: `policy_evaluation` (info-level, every successful evaluation) and `policy_violation` (warning-level, every failed rule or constraint). Exceptions are logged via `CaLog::critical()` before being re-thrown. This provides a unified audit trail without coupling to a specific logging backend.
