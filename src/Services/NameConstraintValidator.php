        ...
        try {
            ...
            if ($subject->emailAddress !== null) {
                if (!$this->isPermitted($subject->emailAddress, NameType::EMAIL)) {
                    $message = "Email address '{$subject->emailAddress}' violates name constraints.";
                    $violations[] = $message;

                    CaLog::log('policy_violation', 'warning', "Policy violation: {$message}", [
                        'subject' => $subject->commonName,
                        'email' => $subject->emailAddress,
                        'constraint_type' => NameType::EMAIL,
                    ]);
                }
            }
            ...
            if ($subject->commonName !== null && $this->looksLikeDomain($subject->commonName)) {
                if (!$this->isPermitted($subject->commonName, NameType::DNS)) {
                    $message = "Common name '{$subject->commonName}' violates DNS name constraints.";
                    $violations[] = $message;

                    CaLog::log('policy_violation', 'warning', "Policy violation: {$message}", [
                        'subject' => $subject->commonName,
                        'constraint_type' => NameType::DNS,
                    ]);
                }
            }
            ...
            if ($sans !== null) {
                foreach ($sans as $san) {
                    ...
                    if (!$this->isPermitted($sanValue, $sanType)) {
                        $message = "SAN {$sanType}:{$sanValue} violates name constraints.";
                        $violations[] = $message;
            ...
            CaLog::log('policy_evaluation', 'info', "Policy evaluated: name_constraint_validation", [
                'subject' => $subject->commonName,
                'san_count' => $sans !== null ? count($sans) : 0,
