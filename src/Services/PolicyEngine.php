...
class PolicyEngine implements PolicyEngineInterface
{
        ...
        try {
            ...
            foreach ($rules as $issuanceRule) {
                ...
                if (!$result->passed) {
                    ...
                    CaLog::log('policy_violation', 'warning', "Policy violation: {$result->message}", [
                        'ca_id' => $ca->id,
                        'rule' => $issuanceRule->name,
                        'rule_class' => $issuanceRule->rule_class,
                        'severity' => $severity,
                        'action' => $action,
                    ]);
            ...
            CaLog::log('policy_evaluation', 'info', "Policy evaluated: evaluate", [
                'ca_id' => $ca->id,
                'certificate_type' => $options->type->slug ?? null,
        ...
        try {
                ...
                if (count($effectivePolicy->allowedKeyUsages) > 0 && count($options->keyUsage) > 0) {
                    foreach ($options->keyUsage as $ku) {
                        if (!in_array($ku, $effectivePolicy->allowedKeyUsages, true)) {
                            ...
                            CaLog::log('policy_violation', 'warning', "Policy violation: Key usage '{$ku}' is not allowed by the CA policy.", [
                                'ca_id' => $ca->id,
                                'key_usage' => $ku,
                            ]);
                        }
                ...
                if (count($effectivePolicy->allowedEkus) > 0 && count($options->extendedKeyUsage) > 0) {
                    foreach ($options->extendedKeyUsage as $eku) {
                        if (!in_array($eku, $effectivePolicy->allowedEkus, true)) {
                            ...
                            CaLog::log('policy_violation', 'warning', "Policy violation: Extended key usage '{$eku}' is not allowed by the CA policy.", [
                                'ca_id' => $ca->id,
            ...
            if (config('ca-policy.validity_enforcement', true)) {
                ...
                if ($effectivePolicy->maxValidityDays > 0 && $options->validityDays > $effectivePolicy->maxValidityDays) {
                    $message = "Requested validity of {$options->validityDays} days exceeds the maximum of {$effectivePolicy->maxValidityDays} days.";
                    $violations[] = $message;

                    CaLog::log('policy_violation', 'warning', "Policy violation: {$message}", [
                        'ca_id' => $ca->id,
                        'requested_days' => $options->validityDays,
                        'max_days' => $effectivePolicy->maxValidityDays,
                    ]);
                }
            ...
            if (config('ca-policy.max_path_length_enforcement', true) && $options->isCa === true) {
                ...
                if ($caPathLength !== null && $caDepth >= $caPathLength) {
                    $message = "CA path length constraint ({$caPathLength}) would be exceeded at chain depth {$caDepth}.";
                    $violations[] = $message;
            ...
            CaLog::log('policy_evaluation', 'info', "Policy evaluated: validateIssuance", [
                'ca_id' => $ca->id,
                'allowed' => $allowed,
                'violations_count' => count($violations),
                'warnings_count' => count($warnings),
        ...
        try {
            ...
            if (config('ca-policy.name_constraints_enabled', true)) {
                ...
                foreach ($constraintViolations as $violation) {
                    CaLog::log('policy_violation', 'warning', "Policy violation: {$violation}", [
                        'ca_id' => $ca->id,
            ...
            CaLog::log('policy_evaluation', 'info', "Policy evaluated: validateSubject", [
                'ca_id' => $ca->id,
                'subject' => $subject->commonName,
                'allowed' => $allowed,
                'violations_count' => count($violations),
    ...
    public function getEffectivePolicy(CertificateAuthority $ca): EffectivePolicy
    {
        try {
            ...
            CaLog::log('policy_evaluation', 'info', "Policy evaluated: getEffectivePolicy", [
                'ca_id' => $ca->id,
                'policy_oid' => $policyOid,
                'rules_count' => count($rules),
                'name_constraints_count' => count($nameConstraints),
            ]);
